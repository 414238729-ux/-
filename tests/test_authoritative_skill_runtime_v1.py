...
def test_implementation_identity_matches_pin() -> None:
    """Current POST-C8 bytes must match their own pin; frozen C8 is independent."""
    from scripts.current_post_c8_implementation_pin import (
        POST_C8_CURRENT_IMPLEMENTATION_IDENTITY,
    )
    from scripts.sgs_engine.formal_duel import implementation_identity

    actual = implementation_identity()
    assert actual == POST_C8_CURRENT_IMPLEMENTATION_IDENTITY
