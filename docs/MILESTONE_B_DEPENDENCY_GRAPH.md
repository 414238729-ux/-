...
| 开始时工作树 | clean |
| 开始时 unmerged | empty |
| 当前 Git 写入状态 | 工作树存在未提交的 Milestone B 实现修改；`commit=null`，`pending` |
| 当前里程碑状态 | `formal_duel_no_skill_ready=true`；`MILESTONE_B=PASSED`（正式160张无技能单挑）；工作树待用户提交与独立审计 |
...
3. 工作树未提交（commit=null/pending）；独立审计尚未发生；不得预写审计 PASSED、未来 commit SHA 或 milestone tag。
