...
阶段 4 仍不得标记为“通过”：里程碑 B 的正式 160 张牌无技能单挑尚未实现。六种基本牌生产批次（里程碑 B1）与多个普通锦囊批次（里程碑 B2）只是步骤，当前阻塞不是缺少测试闭环，而是必须先锁定正式单挑的规则适用范围，并把正式牌堆实际包含的 38 种卡牌逐一接入同一权威状态、事件、响应和动作路径；目前仍有21种（含1种普通锦囊借刀杀人、3种延时锦囊、17种装备）未接入；铁索牌本体已接入但属性伤害传导未完成、不计入完整实现，且 CP-04J 前横置角色受火／雷属性伤害时生产入口失败关闭。不得用测试小牌堆、概率替代或跳过未实现卡牌来伪造里程碑 B。
