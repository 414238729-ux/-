...
本节只记录已发生且可审计的项目事实；不将签入文档冒充为 live Git 状态。
`MILESTONE_B_AUDIT_REMEDIATION_4` 的冻结审计目标为
`3df02b5cfae9af436ba77d8f1c19a7b9959022b1`（fix: close milestone B
remediation 4 pre-audit findings），父提交为
`5d970560e306b84af518798e11db12c2a42dfc44`。Sol Ultra final independent
re-audit 结论为 `MILESTONE_B_REMEDIATION_4_FINAL_REAUDIT_FAILED`；唯一未关闭问题是
`R4-NEW-001` 与 `R4-NEW-002` 两项 MINOR。R3-NEW-001、R3-NEW-002、
R3-NEW-003 与 DOC-OBS-001 在该复审中均为 CLOSED；其中
...
R4 candidate 形成时曾以 `PRECOMMIT`、`NOT_AUDITED_YET`、
`commit=null` 记录当时未提交工作树。这仅是
HISTORICAL/AS-OF（R4 CANDIDATE FORMATION）快照，不表达现在的
Git HEAD、branch 或 worktree。

### LIVE GIT STATE（runtime-derived policy）

branch、HEAD、parent、worktree clean/dirty 与 unmerged 状态必须在审计时由
`git branch --show-current`、`git rev-parse HEAD`、`git status --short` 与
`git ls-files -u` 现场派生。签入文档不对这些可变状态声称 `CURRENT LIVE`。
...
R4 candidate 形成时的修复范围（HISTORICAL/AS-OF；只处理 R3
pre-audit 的 3 项 MINOR + 1 项文档观察）：
...
turn_count=0 仍可 status=passed；full 100-seed 分支忽略 acceptance artifact 的
passed=false 而无条件写 status=passed；implementation bundle digest 未覆盖
structured rule CSV 与 scripts/deck_data.py，修改青龙偃月刀攻击范围后生产语义
改变但 digest 不变）；MB-M-005 MAJOR——TrustedFormalDuelConfiguration 可被公开
import 后直接构造并以 canonical values 自我授予 trusted；MB-M-009 MAJOR——
上述真实缺陷的测试缺失；MB-M-010 MAJOR——manifest/docs current 状态互斥或过时
（旧 HEAD、commit=null/pending/dirty、ready 与 blocked 并存、R1 仍写
NOT_AUDITED_YET/PRECOMMIT_READY、Remediation 2 未提交写成 CURRENT）；
R2-NEW-001 MINOR——execution hash 未覆盖 damage_card_already_finished／
pending_slash_choice／pending_discard_two／pending_hanbing_discard 四个
runtime 字段（只改 pending_slash_choice.pending_slash.boosted 时 hash 不变但
后续伤害 1→2）。

【HISTORICAL/AS-OF（PRECOMMIT SNAPSHOT）】R1 开始时 HEAD=`ebd656754ed2a528e0d08cd5175b68385fdb8140`，当时工作树尚未提交、`commit=null`、`pending`、`independent_audit_done=false`、
`audit_conclusion=NOT_AUDITED_YET`；R2 开始时 HEAD=`8ce4970...`，R2 工作树当时未提交。二者均为提交前工作树快照，不表达当前状态。本节不是 whole-repo audit 的续写，也不是
`remediation-7`。
...
global_complete_card_key_count=37
global_complete_instance_count=159
duel_complete_card_key_count=38
...
正式100-seed验收 `FORMAL_MILESTONE_B_100_SEED_ACCEPTANCE`（schema v2、seeds 0..99、formal profile、analysis_only=false、max_steps=2000、每局同一 session 现场执行并同步 record decisions、strict replay 同一份 record + final_state_hash）已多次重新现场运行（R1 2026-08-11 2631.2s；R2 2026-08-12 1269.5s；R3 2026-08-13 见下方本地验证）：100/100 自然结束、failures=0、safety-cap 0、unsupported 0、approximation 0；artifact=docs/FORMAL_MILESTONE_B_100_SEED_ACCEPTANCE.json。artifact 绑定当前 implementation/rules-profile/deck identity，只是缓存证据；live gate 现场严格验证全部关键字段（MB-B-001），`simulation_executed=false` 的 status 绝不把缓存称为已执行。R2 期间还从头运行一次 independent determinism reproduction（相同 0..99、新 session secrets）：winner mismatch=0、action_count mismatch=0、final canonical hash mismatch=0。
...
【HISTORICAL/AS-OF（R3 candidate 形成）】R3 修复范围：MB-B-001——建立唯一 canonical live-result
validator（`validate_formal_live_result`／`validate_formal_live_result_set`），
acceptance generator／formal runner／live result writer／readiness cached
consumer 全部复用；逐 seed 验证 seed 整数、winner∈{p1,p2}、natural_end、
strict replay、unsupported/approximation=0、deck_count=160、action_count>0、
turn_count>0、final_state_hash 精确 64-hex；集合级由 details 重新派生
failures/passed；runner 移除“跑完即 passed”隐式逻辑，不再覆写 validator；
implementation bundle 纳入 structured rule CSV 与 scripts/deck_data.py
（FORMAL_SIMULATION_TRANSITIVE_INPUT_INVENTORY，Git-normalized SHA-256，
行尾归一、docs/artifact/manifest 排除）。MB-M-005——trusted capability
私有 sentinel（`_TRUSTED_FORMAL_CAPABILITY`）：TrustedFormalDuelConfiguration
公开直接构造／forged token 均失败关闭，copy/deepcopy 只能经内部 canonical
factory，replace 篡改失败关闭，正式会话校验真实 capability identity 而非
仅 isinstance；replay 只保存 profile value，strict re-execution 由
from_canonical_profile_value 经内部 factory 重建 trusted。MB-M-009——新增
runner 验证矩阵（malformed hash／deck_count／action_count／turn_count／
passed=false／missing／duplicate seed 等）、provenance、identity 依赖、
execution hash mutation 测试。MB-M-010——manifest/docs CURRENT/HISTORICAL
收口：R2 re-audit FAILED 回填、R3 仅 NOT_AUDITED_YET、CURRENT 只写当前树
可确定事实。R2-NEW-001——execution snapshot 覆盖 damage_card_already_finished
／pending_slash_choice／pending_discard_two／pending_hanbing_discard，
`EXECUTION_HASH_RUNTIME_INVENTORY` 登记 A 类行为字段，
pending_slash_choice.pending_slash 与 runtime.pending_slash 一致性 invariant
（分叉失败关闭，不允许无声双份）。R2 re-audit 已确认无回滚项
（MB-B-002/003/004、MB-M-006/007/008、MB-N-011/012/013、R1-NEW-001/002/003）
保持；不重新设计已 CLOSED 部分。

三件原 PARTIAL 武器的当前边界：

- 雌雄双股剑：通用异性目标状态机与 `CharacterGender.NONE` 支持已实现，卡牌通用语义为 `COMPLETE`；formal soldier profile 提供权威参与者（双方 NONE，不构成异性），不再是模式缺口。
- 丈八蛇矛：材料生命周期已由用户确认并实现（`HAND→PROCESSING→DISCARD`），`WEAPON_SKILL_STATUS=COMPLETE`；虚拟杀动作携带类型化 `VirtualCardReference`（公共枚举器校验材料存在且不重复，未恢复全局 `virtual:` 豁免）。
- 方天画戟：全局多人／多目标语义仍为 `PARTIAL`；严格两人 duel 中不存在第三名合法目标，该分支为 `NOT_APPLICABLE_TO_DUEL`。这只证明 duel-scope 充分，不得改写为 global `COMPLETE`。
...
【HISTORICAL/AS-OF（R4 candidate 形成时验证）】下列均为当时本地
实现/回归证据，不是独立审计：见
...
passed（2648s）；第二 determinism reproduction mismatch 均 0（288.7s）。
【HISTORICAL/AS-OF（R2 2026-08-12 验证）】完整 pytest `2118 passed`
...
- R4（WHOLE_REPO_AUDIT_REMEDIATION_4，remediation-3 targeted independent re-audit 失败后的 docs-only 治理收尾轮；
  ...
  父提交 6bef086...）已由用户在外部PowerShell创建；检查点状态：status=committed_pending_audit、checkpoint_commit=null
  （尚未产生）、milestone_tag=null、worktree_commit_pending=true；independent_audit_done=true、
  audit_conclusion=REMEDIATION_4_REAUDIT_FAILED（HISTORICAL/AS-OF：R4 文档回填时为 NOT_AUDITED_YET；CURRENT LIVE REF：
  remediation-4 targeted independent re-audit 已进行且结论为 REMEDIATION_4_REAUDIT_FAILED，原因是当时唯一残余
  current-state contradiction——ENGINE_STATUS 顶部>状态：总述中 R2 G-002 子句未分层；R4 re-audit 已确认
  G-004=CLOSED、G-005=STILL_CLOSED、R2-NEW-001=STILL_CLOSED，并确认 G-001/G-002/G-003/QILIN_TIMING_BUG/
  R1-NEW-001/R1-NEW-002=NO_REGRESSION，但 R4 整体不视为 PASSED，不得改写为 PASSED）。历史五次失败结论
  （WHOLE_REPO_AUDIT_FAILED／REMEDIATION_REAUDIT_FAILED／REMEDIATION_2_REAUDIT_FAILED／
  REMEDIATION_3_REAUDIT_FAILED／REMEDIATION_4_REAUDIT_FAILED）均保持，不得写任何 PASSED／WHOLE_REPO_AUDIT_PASSED。
- R5（WHOLE_REPO_AUDIT_REMEDIATION_5，remediation-4 targeted independent re-audit 失败后的单点最终收尾轮；
  实现提交后 checkpoint 记录）：
  R4-NEW-001／R3-NEW-001 residual——ENGINE_STATUS 顶部>状态：总述中 R2 G-002 子句双层分层：旧 current 歧义
  表述“G-002本地source gap清零[pending independent re-audit]”已改为
  HISTORICAL/AS-OF：R2 implementation 完成、独立 targeted re-audit 尚未发生时状态为
  G-002_SOURCE_GAP_LOCALLY_RESOLVED_PENDING_INDEPENDENT_REAUDIT；CURRENT：R2 targeted independent re-audit
  已确认 G-002=CLOSED；R2 整体=REMEDIATION_2_REAUDIT_FAILED（因当时 G-004/G-005 仍 OPEN）保持，
  不得把 R2 整体 FAILED 与 G-002 单项 CLOSED 混淆。
  R4 re-audit 已确认 G-004=CLOSED、G-005=STILL_CLOSED、R2-NEW-001=STILL_CLOSED，并确认
  G-001/G-002/G-003/QILIN_TIMING_BUG/R1-NEW-001/R1-NEW-002=NO_REGRESSION，本轮保持；
  本轮为 docs-only（implementation 仅修改 docs/ENGINE_STATUS.md，1 insertion/1 deletion，
  runtime/gameplay files modified=0），未重新运行 pytest（1974 passed 为既有 implementation/regression evidence）。
  验证（docs 治理证据）：JSON parse 通过；SHA-256 38 entries mismatch=0；docs consistency scan current-state
  contradiction=0；git diff --check passed；git ls-files -u empty。
  实现提交 2c28cf0404f4b7bbc0752fa75594580fafef1256（docs: resolve final audit status contradiction，
  父提交 bf5d83c...）已由用户在外部PowerShell创建；检查点状态：status=committed_pending_audit、checkpoint_commit=null
  （尚未产生）、milestone_tag=null、worktree_commit_pending=true；independent_audit_done=true、
  audit_conclusion=REMEDIATION_5_REAUDIT_FAILED（HISTORICAL/AS-OF：R5 文档回填时为 NOT_AUDITED_YET；CURRENT LIVE REF：
  remediation-5 targeted independent re-audit 已进行且结论为 REMEDIATION_5_REAUDIT_FAILED，唯一新 finding
  R5-NEW-001 MINOR——manifest machine-readable generic current field g002_source_gap_locally_resolved 仍表达
  G-002 pending；R5 re-audit 已确认 R4-NEW-001=CLOSED、R3-NEW-001 residual=CLOSED、G-004=STILL_CLOSED、
  G-005=STILL_CLOSED、R2-NEW-001=STILL_CLOSED，并确认 G-001/G-002/G-003/QILIN_TIMING_BUG/R1-NEW-001/
  R1-NEW-002 全部 NO_REGRESSION，G-002 source finding 本身仍=CLOSED，但 R5 整体不视为 PASSED，
  不得改写为 PASSED）。历史六次失败结论（WHOLE_REPO_AUDIT_FAILED／REMEDIATION_REAUDIT_FAILED／
  REMEDIATION_2_REAUDIT_FAILED／REMEDIATION_3_REAUDIT_FAILED／REMEDIATION_4_REAUDIT_FAILED／
  REMEDIATION_5_REAUDIT_FAILED）均保持，不得写任何 PASSED／WHOLE_REPO_AUDIT_PASSED。
- R6（WHOLE_REPO_AUDIT_REMEDIATION_6，remediation-5 targeted independent re-audit 失败后的 manifest-only
  单字段治理收尾轮；实现提交后 checkpoint 记录）：
  R5-NEW-001 MINOR——final_verification.whole_repo_audit_remediation_2.g002_source_gap_locally_resolved 值由
  G-002_SOURCE_GAP_LOCALLY_RESOLVED_PENDING_INDEPENDENT_REAUDIT 改为 G-002_CLOSED（key 保持不变）；
  字段消费者搜索确认仅 manifest 自身使用（无 scripts/tests/schema 消费者）；历史 pending 事实继续由明确
  HISTORICAL/AS-OF 的 R2 历史 checkpoint 记录承载，未新增任何 generic current scalar 重新表达 G-002 pending。
  R5 re-audit 已确认 CLOSED/STILL_CLOSED/NO_REGRESSION 项保持；本轮无 runtime/gameplay 修改
  （runtime/gameplay files modified=0），未重新运行 pytest（1974 passed 仅作为既有 implementation/regression evidence）。
  验证（docs 治理证据）：JSON parse 通过；SHA-256 38 entries mismatch=0；manifest machine-state consistency scan
  通过（g002_source_gap_locally_resolved 唯一值=G-002_CLOSED；所有 PENDING_INDEPENDENT_REAUDIT 命中均明确
  historical/as-of）；four-doc current-state contradiction scan=0；git diff --check passed；git ls-files -u empty。
  实现提交 2a075580484fadf3a2e4b7b76f4e4ad918af66e3（docs: normalize G-002 machine-readable audit state，
  父提交 f838488...）已由用户在外部PowerShell创建；检查点状态：status=audited、independent_audit_done=true、
  audit_conclusion=REMEDIATION_6_REAUDIT_PASSED、checkpoint_commit=null（尚未产生）、milestone_tag=null、
  worktree_commit_pending=false（独立 targeted re-audit 已发生：WHOLE_REPO_AUDIT_REMEDIATION_6_TARGETED_REAUDIT，
  最终结论 REMEDIATION_6_REAUDIT_PASSED；finding closure——R5-NEW-001=CLOSED、R4-NEW-001=STILL_CLOSED、
  R3-NEW-001 residual=STILL_CLOSED、G-004=STILL_CLOSED、G-005=STILL_CLOSED、R2-NEW-001=STILL_CLOSED；
  R2_CLOSED_FINDINGS_REGRESSION_CHECK 六项（G-001/G-002/G-003/QILIN_TIMING_BUG/R1-NEW-001/R1-NEW-002）全部
  NO_REGRESSION；NEW_FINDINGS=NONE；章节 18_KNOWLEDGE_AND_DOCS_CONSISTENCY=19_HISTORICAL_FIX_PROPAGATION=
  25_REMEDIATION_6_MANIFEST_ONLY_CLEANUP_INTEGRITY=PASSED，关键 PARTIAL_READ=0、关键 NOT_READ=0；
  被审计 checkpoint=95d068dd01a97ae4337696a25df4edb1b317f26b（docs: record sixth whole-repo audit remediation checkpoint）、
  implementation=2a075580484fadf3a2e4b7b76f4e4ad918af66e3、audit_result_commit=0e023694df7ea123c41c2b415784532c9d185d26（docs: record successful whole-repo remediation re-audit）（已回填）、
  milestone_tag=null、worktree_commit_pending=false）。历史链保持：original=WHOLE_REPO_AUDIT_FAILED、R1-R5=REMEDIATION_REAUDIT_FAILED／…／
  REMEDIATION_5_REAUDIT_FAILED（不得压平）；当前通过的是 remediation chain 最终独立复审，
  不得把 WHOLE_REPO_AUDIT_FAILED 改写成 WHOLE_REPO_AUDIT_PASSED；R6 re-audit 为静态/仓库审计证据，
  R6 未重新运行 pytest（1974 passed 仍为既有 implementation/regression evidence），两类证据不得混写。
