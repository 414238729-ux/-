...
本节记录 `MILESTONE_B_FORMAL_160_CARD_NO_SKILL_DUEL_ADVANCEMENT` 工作树的当前事实，优先于下方较早批次快照。它建立在固定基线 `4c8c4466f1950740c69767a01a0b24cab723b310`（`docs: correct finalization governance residuals`）之后；开发分支为 `sol-ultra-milestone-b-formal-duel`，本轮尚未提交，`commit=null`、`pending`。本节不是 whole-repo audit 的续写，也不是 `remediation-7`。
...
global_complete_card_key_count=36
global_complete_instance_count=158
duel_complete_card_key_count=37
duel_complete_instance_count=159
...
当前实现继续复用 `ProductionBasicCardBatch` 的同一 `GameState`、`CardInstance`、事件、合法动作、响应窗口、伤害、距离、阶段、濒死、胜负、重洗与 `DeterministicRNG`；`FormalNoSkillDuelSession` 只是薄模式层，没有复制 `test_only_duel_vertical_slice`，也不存在第二套正式引擎。analysis-only factory 已可到达该生产核心，formal 严格重执行 replay 能力已接入。runner 的 run 分支本身已经实现：先经过 live gate，再复检 canonical seeds 0..99 的逐 seed 证据，只有全部合格才原子写入 JSON；当前 `_FORMAL_EXECUTION_RELEASED=false`、规则 profile／卡牌／100-seed 前置条件未关闭，因此 live gate 会在接触输出路径前失败关闭，正式验收结果仍为 0。

三件原 PARTIAL 武器的当前边界：

- 雌雄双股剑：通用异性目标状态机与通用 `CharacterMetadata`／`CharacterGender` 已实现，卡牌通用语义为 `COMPLETE`；formal 参与者的角色 key 与性别仍没有权威规则来源，因此保留 `MODE_GAP`，不得按座次硬编码。
- 丈八蛇矛：保持 global／duel 阻塞。Knowledge 尚不能裁定两张材料牌是 A（`HAND→PROCESSING→结算后DISCARD`）还是 B（使用／打出开始即 `HAND→DISCARD`、从不进入 `PROCESSING`），因此仍为 `RULE_SOURCE_GAP`。通用 `VirtualCardReference` foundation 已实现并进入 `LegalAction`：显式绑定 `card_key`、`conversion_rule_id` 与材料实体 ID，禁止和 physical `card_instance_id` 同时存在，并绑定公共枚举校验、动作 ID 及 strict production／duel replay；原独立 `DATA_MODEL_GAP` 已关闭。丈八具体 lifecycle 接线只在 A／B 规则确认后进行。
- 方天画戟：全局多人／多目标语义仍为 `PARTIAL`；严格两人 duel 中不存在第三名合法目标，该分支为 `NOT_APPLICABLE_TO_DUEL`。这只证明 duel-scope 充分，不得改写为 global `COMPLETE`。

当前两个规则源计数项是 formal duel 权威 profile 与丈八材料生命周期。formal profile 至少还须确认平台／版本、160 张牌堆适用性、初始手牌与体力、先手及首回合摸牌、同将／手气卡规则、参与者选择与角色性别来源。没有这些确认时，`analysis_convention` 只能用于诊断，不能生成正式结果。

已生成 [MILESTONE_B_ANALYSIS_SEED_DIAGNOSTIC.json](MILESTONE_B_ANALYSIS_SEED_DIAGNOSTIC.json) 作为非正式诊断证据：固定 seeds 0..99 全部保留、无排除或重采样、每局 `max_steps=2000`；100 条记录中 55 局自然结束（p1=34、p2=21，动作数 44..494），45 局只因预期的 `UnsupportedRuleError` 失败关闭（丈八生命周期 23、雌雄 formal 角色／性别元数据 22）。所有局均为 160 张牌，联合到达 38 个 card key；safety-cap、`InvalidActionError`、`ProductionBatchError` 和其他异常均为 0，说明本轮观察到的非规则 seed blocker 已清零。但诊断使用未确认 profile，每个 seed 至少记录 1 个 unsupported 与 1 个 approximation，总计 unsupported=145、approximation=100；`formal_result_eligible=0`、`reexecution_verified=0`，因此它不是 formal acceptance。live readiness 继续保持 `acceptance_seed_count=0`、`unsupported_rules=2`、`approximation_count=0`，两套计数口径不得混写。

本轮本地验证已经收口：`python -m pytest -q` 为 `2024 passed, 1 warning in 830.59s`（唯一 warning 是 `.pytest_cache` 的 Windows `WinError 5`，无测试失败）；`python -m compileall -q scripts tests` exit 0；source integrity exit 0，扫描125个 Python 文件，117项 finding／audit item（formal source 56、test code 61），`defect_count=0`；manifest 与诊断 JSON 均可解析，诊断 seed ID 严格等于0..99；runner status exit 0 并现场返回上方160/38、36/158、37/159及 blocked readiness；Git-normalized SHA-256 表47个文件条目全部匹配；`git diff --check` exit 0（仅 Git 的 LF→CRLF 提示）。`VirtualCardReference` 所在 `tests/test_sgs_engine_actions.py` 定向18项亦已通过。以上是本地实现／回归证据，不是独立审计，也不能把 analysis-only 诊断转成正式100-seed acceptance。独立审计尚未运行：`independent_audit_done=false`、`audit_conclusion=NOT_AUDITED_YET`、`milestone_tag=null`。当前结论仍为 `MILESTONE_B_BLOCKED`。
